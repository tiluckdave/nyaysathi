...
const getLawyer = async () => {
  ...
  const userRef = doc(db, 'users', uid);
  const userData = await getDoc(userRef);
  const LawyerRef = doc(db, 'lawyers', uid + userData.data().email);
  const LawyerData = await getDoc(LawyerRef)
  console.log(LawyerData.data())
  return LawyerData.data();
}

const formatLawyer = async (user) => {
  const previousLawyer = await getLawyer();
  return {
    uid: previousLawyer?.uid || user.uid + user.email,
    ...
    email: previousLawyer?.email || user.email,
